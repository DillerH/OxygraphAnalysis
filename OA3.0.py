...
def importData(): 
    ...
    for fileName in trialList:
        ...
        if fileName != '.DS_Store':
            ...
            for j in range(29,len(strlist)):
                temp = strlist[j].split(",")
                timeList.append(float(temp[0]))
                dataList.append(float(temp[1]))
                flagList.append(str(temp[3]))
                
                if str(temp[3])=='Stop':
                    break
            ziplist = zip(timeList,dataList,flagList)
            dfList.append(pd.DataFrame(ziplist,columns=['Time', 'Oxygen 1', 'Label']))
...
def barPlot(dfSlopes):
    ...
    for i in slopeStrs:
        ...
        AvgSlopes.append(rowSlice.mean())
        standarderrors.append(rowSlice.std())
...
def makeDirs():
    ...
    try:
        ...
        print('Directory Already Present, Overwrite Eminent')
...
def fixTime(dfList):
    ...
    for df_it in dfList:   
        ...
        df_it['time'] = df_it['Time'].astype('float',copy=False) - timeStart
        df_temp = df_it[df_it['Label'].isin(slopeStrs)]
        df_it = df_it.query('time >= 0')
...
def main():
    ...
    for i in range(0,len(dfList)):
        ...
        for k in range(0,len(slopeStrs)):
            slopeList = trimSlice(sliceList[k],slopeStrs[k], i, slopeList) #faulty line
